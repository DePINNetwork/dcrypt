...
# 4) Remove any existing output file
[ -f "$output" ] && rm "$output"
...
print_tree() {
  ...
  for item in $items; do
    ...
    # Exclusions
    # -------------------------------------------------------------------------
    # 1) Skip anything under 'sandbox', 'tools', or 'target' folders
    if [[ "$full_path" == "$dir/sandbox"* || "$full_path" == "$dir/tools"* || "$full_path" == "$dir/target"* ]]; then
      continue
    fi
    # 2) Skip .ico files
    if [[ "$item" == *.ico ]]; then
...
# 5) Write a top-level entry for the directory itself, then recurse
