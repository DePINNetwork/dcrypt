...
impl TimingTester {
    ...
    pub fn cohens_d(times_a: &[u128], times_b: &[u128]) -> f64 {
        ...
        let pooled_std_dev = ((var_a * (n_a - 1.0) + var_b * (n_b - 1.0)) / 
                              (n_a + n_b - 2.0)).sqrt();
    ...
    pub fn combined_score(&self, mean_ratio: f64, _p_value: f64, t_stat: f64, rel_std_dev_a: f64, rel_std_dev_b: f64) -> f64 {
    ...
    pub fn analyze_constant_time(
        ...
        let mean_ratio = if mean_a > mean_b { mean_a / mean_b } else { mean_b / mean_a };
...
pub fn generate_test_insights(analysis: &TimingAnalysis, _config: &crate::suites::constant_time::config::TestConfig, primitive_name: &str) -> String {
    let mut insights = String::new();
    
    // Calculate additional metrics for analysis
    let mean_diff = (analysis.mean_a - analysis.mean_b).abs();
    let timing_difference_percent = (mean_diff / f64::min(analysis.mean_a, analysis.mean_b)) * 100.0;
    let rel_std_dev_a = analysis.std_dev_a / analysis.mean_a;
    let rel_std_dev_b = analysis.std_dev_b / analysis.mean_b;
    let std_dev_ratio = f64::max(rel_std_dev_a, rel_std_dev_b) / f64::min(rel_std_dev_a, rel_std_dev_b);
    
    // Test status with minimal output for passing tests
    if analysis.is_constant_time {
        insights.push_str(&format!("✅ PASS: {} implementation appears to be constant-time.\n", primitive_name));
        return insights;
    }
    ...
    insights.push_str(&format!("  Mean: {:.0} ns vs {:.0} ns (diff: {:.1}%)\n", 
                              analysis.mean_a, analysis.mean_b, timing_difference_percent));
    insights.push_str(&format!("  Stats: t={:.1}, p={:.2e}, d={:.1}\n\n", 
    ...
    if timing_difference_percent > 40.0 && analysis.cohens_d > 5.0 {
        ...
        insights.push_str("  ⚠️ Large timing difference suggests operations being skipped for certain inputs\n");
    ...
    if std_dev_ratio > 4.0 && (rel_std_dev_a > 0.1 || rel_std_dev_b > 0.1) {
        ...
        insights.push_str("  ⚠️ Disparity in timing variability indicates data-dependent loop counts\n");
    ...
    if (rel_std_dev_a < 0.05 && rel_std_dev_b < 0.05) && analysis.mean_ratio > 1.2 {
        ...
        insights.push_str("  🛠️ Ensure data accesses follow fixed patterns independent of secrets\n");
    }
    
    // Add a fallback if no specific patterns were detected
    if !patterns_detected {
        insights.push_str("🔍 General timing inconsistency\n");
        insights.push_str("  ⚠️ Pattern doesn't match known signatures but timing is not constant\n");
