...
pub(crate) fn ecdsa_keygen(group: &TestGroup, case: &TestCase) -> Result<()> {
    let curve = group.defaults.get("curve")
    ...
    match curve.as_str() {
        "P-192" | "secp192r1" => { // Added P-192
            let (public_key, secret_key) = EcdsaP192::keypair(&mut rng)
                .map_err(|e| EngineError::Crypto(format!("P-192 keypair generation failed: {:?}", e)))?;
            ...
            if pub_bytes.len() == ec_p192::P192_POINT_UNCOMPRESSED_SIZE && pub_bytes[0] == 0x04 {
                case.outputs.borrow_mut().insert("d".into(), hex::encode(d_bytes));
                case.outputs.borrow_mut().insert("qx".into(), hex::encode(&pub_bytes[1..1 + ec_p192::P192_FIELD_ELEMENT_SIZE]));
                case.outputs.borrow_mut().insert("qy".into(), hex::encode(&pub_bytes[1 + ec_p192::P192_FIELD_ELEMENT_SIZE..]));
            } else {
                return Err(EngineError::Crypto("Invalid P-192 public key format".into()));
            }
        }
        "P-224" | "secp224r1" => {
            let (public_key, secret_key) = EcdsaP224::keypair(&mut rng)
                .map_err(|e| EngineError::Crypto(format!("P-224 keypair generation failed: {:?}", e)))?;
            let d_bytes = secret_key.as_ref();
            let pub_bytes = public_key.as_ref();
            if pub_bytes.len() == 57 && pub_bytes[0] == 0x04 {
                case.outputs.borrow_mut().insert("d".into(), hex::encode(d_bytes));
                case.outputs.borrow_mut().insert("qx".into(), hex::encode(&pub_bytes[1..29]));
                case.outputs.borrow_mut().insert("qy".into(), hex::encode(&pub_bytes[29..57]));
            } else {
                return Err(EngineError::Crypto("Invalid P-224 public key format".into()));
            }
        }
        "P-256" | "secp256r1" => {
            let (public_key, secret_key) = EcdsaP256::keypair(&mut rng)
                .map_err(|e| EngineError::Crypto(format!("P-256 keypair generation failed: {:?}", e)))?;
            let d_bytes = secret_key.as_ref();
            let pub_bytes = public_key.as_ref();
            if pub_bytes.len() == 65 && pub_bytes[0] == 0x04 {
                case.outputs.borrow_mut().insert("d".into(), hex::encode(d_bytes));
                case.outputs.borrow_mut().insert("qx".into(), hex::encode(&pub_bytes[1..33]));
                case.outputs.borrow_mut().insert("qy".into(), hex::encode(&pub_bytes[33..65]));
            } else {
                return Err(EngineError::Crypto("Invalid P-256 public key format".into()));
            }
        }
        "P-384" | "secp384r1" => {
            let (public_key, secret_key) = EcdsaP384::keypair(&mut rng)
                .map_err(|e| EngineError::Crypto(format!("P-384 keypair generation failed: {:?}", e)))?;
            let d_bytes = secret_key.as_ref();
            let pub_bytes = public_key.as_ref();
            if pub_bytes.len() == 97 && pub_bytes[0] == 0x04 {
                case.outputs.borrow_mut().insert("d".into(), hex::encode(d_bytes));
                case.outputs.borrow_mut().insert("qx".into(), hex::encode(&pub_bytes[1..49]));
                case.outputs.borrow_mut().insert("qy".into(), hex::encode(&pub_bytes[49..97]));
            } else {
                return Err(EngineError::Crypto("Invalid P-384 public key format".into()));
            }
        }
        "P-521" | "secp521r1" => {
            let (public_key, secret_key) = EcdsaP521::keypair(&mut rng)
                .map_err(|e| EngineError::Crypto(format!("P-521 keypair generation failed: {:?}", e)))?;
            let d_bytes = secret_key.as_ref();
            let pub_bytes = public_key.as_ref();
            if pub_bytes.len() == 133 && pub_bytes[0] == 0x04 {
                case.outputs.borrow_mut().insert("d".into(), hex::encode(d_bytes));
                case.outputs.borrow_mut().insert("qx".into(), hex::encode(&pub_bytes[1..67]));
                case.outputs.borrow_mut().insert("qy".into(), hex::encode(&pub_bytes[67..133]));
            } else {
                return Err(EngineError::Crypto("Invalid P-521 public key format".into()));
            }
        }
        ...
        s if s.starts_with("K-") || s.starts_with("B-") => {
            ...
            case.outputs.borrow_mut().insert("reason".into(), format!("Unsupported curve: {}", s));
            return Ok(());
        }
...
pub(crate) fn ecdsa_keyver(group: &TestGroup, case: &TestCase) -> Result<()> {
    ...
    let qx_hex = case.inputs.get("qx")
        .map(|v| v.as_string())
        .ok_or(EngineError::MissingField("qx"))?;
    let qy_hex = case.inputs.get("qy")
        .map(|v| v.as_string())
    ...
    let is_valid = match curve.as_str() {
        ...
        s if s.starts_with("K-") || s.starts_with("B-") => {
            ...
            case.outputs.borrow_mut().insert("testPassed".into(), "false".into()); // Treat as not passed for this specific test case
            case.outputs.borrow_mut().insert("reason".into(), format!("Unsupported curve for keyVer: {}", s));
            return Ok(());
        }
        _ => return Err(EngineError::Crypto(format!("Unsupported curve: {}", curve))),
    };
...
pub(crate) fn ecdsa_siggen(group: &TestGroup, case: &TestCase) -> Result<()> {
    ...
    let msg_hex = case.inputs.get("message")
    ...
    match curve.as_str() {
        "P-192" | "secp192r1" => { // Added P-192
            let (public_key, secret_key) = EcdsaP192::keypair(&mut rng)
                .map_err(|e| EngineError::Crypto(format!("P-192 keypair generation failed for siggen: {:?}", e)))?;
            ...
            case.outputs.borrow_mut().insert("d".into(), hex::encode(secret_key.as_ref()));
            let (r, s) = parse_der_signature(signature.as_ref())?;
            case.outputs.borrow_mut().insert("r".into(), hex::encode(&r));
            case.outputs.borrow_mut().insert("s".into(), hex::encode(&s));
        }
        "P-224" | "secp224r1" => {
            let (public_key, secret_key) = EcdsaP224::keypair(&mut rng)
                .map_err(|e| EngineError::Crypto(format!("P-224 keypair generation failed for siggen: {:?}", e)))?;
        ...
        "P-256" | "secp256r1" => {
            let (public_key, secret_key) = EcdsaP256::keypair(&mut rng)
                .map_err(|e| EngineError::Crypto(format!("P-256 keypair generation failed for siggen: {:?}", e)))?;
        ...
        "P-384" | "secp384r1" => {
            let (public_key, secret_key) = EcdsaP384::keypair(&mut rng)
                .map_err(|e| EngineError::Crypto(format!("P-384 keypair generation failed for siggen: {:?}", e)))?;
        ...
        "P-521" | "secp521r1" => {
            let (public_key, secret_key) = EcdsaP521::keypair(&mut rng)
                .map_err(|e| EngineError::Crypto(format!("P-521 keypair generation failed for siggen: {:?}", e)))?;
        ...
        s if s.starts_with("K-") || s.starts_with("B-") => {
            ...
            case.outputs.borrow_mut().insert("testPassed".into(), "false".into());
            case.outputs.borrow_mut().insert("reason".into(), format!("Unsupported curve for sigGen: {}", s));
            return Ok(());
        }
        _ => return Err(EngineError::Crypto(format!("Unsupported curve: {}", curve))),
    }
...
pub(crate) fn ecdsa_sigver(group: &TestGroup, case: &TestCase) -> Result<()> {
    ...
    let r_hex = case.inputs.get("r")
        .map(|v| v.as_string())
        .ok_or(EngineError::MissingField("r"))?;
    let s_hex = case.inputs.get("s")
        .map(|v| v.as_string())
    ...
    let result = match curve.as_str() {
        ...
        s if s.starts_with("K-") || s.starts_with("B-") => {
            ...
            case.outputs.borrow_mut().insert("testPassed".into(), "false".into());
             case.outputs.borrow_mut().insert("reason".into(), format!("Unsupported curve for sigVer: {}", s));
            return Ok(());
        }
        _ => return Err(EngineError::Crypto(format!("Unsupported curve: {}", curve))),
    };
...
/* Helper functions */
fn verify_p192_public_key(qx: &[u8], qy: &[u8]) -> bool { // Added P-192 helper
...
fn verify_p224_public_key(qx: &[u8], qy: &[u8]) -> bool {
    if qx.len() != 28 || qy.len() != 28 { // P-224 coordinates are 28 bytes
        return false;
    }
    let mut point = vec![0x04];
    point.extend_from_slice(qx);
...
fn create_p192_public_key(qx_hex: &str, qy_hex: &str) -> Result<EcdsaP192PublicKey> { // Added P-192 helper
    let qx_bytes = hex::decode(qx_hex)?;
    let qy_bytes = hex::decode(qy_hex)?;
    if qx_bytes.len() != ec_p192::P192_FIELD_ELEMENT_SIZE || qy_bytes.len() != ec_p192::P192_FIELD_ELEMENT_SIZE {
        return Err(EngineError::InvalidData("Invalid P-192 public key component size".into()));
    }
...
fn create_p224_public_key(qx_hex: &str, qy_hex: &str) -> Result<EcdsaP224PublicKey> {
    ...
    if qx_bytes.len() != 28 || qy_bytes.len() != 28 {
        return Err(EngineError::InvalidData("Invalid P-224 public key component size".into()));
    }
    let mut key = [0u8; 57];
    key[0] = 0x04;
...
fn create_p256_public_key(qx_hex: &str, qy_hex: &str) -> Result<EcdsaP256PublicKey> {
    ...
    if qx_bytes.len() != 32 || qy_bytes.len() != 32 {
        return Err(EngineError::InvalidData("Invalid P-256 public key component size".into()));
    }
    let mut key = [0u8; 65];
    key[0] = 0x04;
...
fn create_p384_public_key(qx_hex: &str, qy_hex: &str) -> Result<EcdsaP384PublicKey> {
    ...
    if qx_bytes.len() != 48 || qy_bytes.len() != 48 {
        return Err(EngineError::InvalidData("Invalid P-384 public key component size".into()));
    }
    let mut key = [0u8; 97];
    key[0] = 0x04;
...
fn create_p521_public_key(qx_hex: &str, qy_hex: &str) -> Result<EcdsaP521PublicKey> {
    ...
    if qx_bytes.len() != 66 || qy_bytes.len() != 66 {
        return Err(EngineError::InvalidData("Invalid P-521 public key component size".into()));
    }
    let mut key = [0u8; 133];
    key[0] = 0x04;
