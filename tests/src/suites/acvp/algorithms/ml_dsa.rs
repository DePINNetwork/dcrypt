...
pub(crate) fn ml_dsa_keygen(group: &TestGroup, case: &TestCase) -> Result<()> {
    ...
    if let Some(sk_hex) = case.inputs.get("sk").map(|v| v.as_string()) {
        ...
        let sk_output = match (param_set, sk_bytes.len()) {
            ...
            _ => AcvpSecretKeyAdapter::to_acvp_bytes(&sk, param_set)
                .map_err(|e| EngineError::Crypto(e.to_string()))?,
        };

        case.outputs
            .borrow_mut()
            .insert("pk".into(), hex::encode(pk.to_bytes()));
        case.outputs
            .borrow_mut()
            .insert("sk".into(), hex::encode(sk_output));
    } else {
        ...
        let (pk, sk) = match param_set {
            "Dilithium2" => {
                let (pk, sk) = Dilithium2::keypair(&mut rng).map_err(|e| {
                    EngineError::Crypto(format!("Dilithium2 keypair generation failed: {:?}", e))
                })?;
                (pk, sk)
            }
            "Dilithium3" => {
                let (pk, sk) = Dilithium3::keypair(&mut rng).map_err(|e| {
                    EngineError::Crypto(format!("Dilithium3 keypair generation failed: {:?}", e))
                })?;
                (pk, sk)
            }
            "Dilithium5" => {
                let (pk, sk) = Dilithium5::keypair(&mut rng).map_err(|e| {
                    EngineError::Crypto(format!("Dilithium5 keypair generation failed: {:?}", e))
                })?;
                (pk, sk)
            }
            _ => unreachable!(),
        ...
        case.outputs
            .borrow_mut()
            .insert("sk".into(), hex::encode(acvp_sk));
    }
...
pub(crate) fn ml_dsa_siggen(group: &TestGroup, case: &TestCase) -> Result<()> {
    ...
    if let Some(sk_hex) = case.inputs.get("sk").map(|v| v.as_string()) {
        ...
        case.outputs
            .borrow_mut()
            .insert("signature".into(), hex::encode(sig.to_bytes()));
    } else {
        ...
        let (pk, sk, sig) = match param_set {
            "Dilithium2" => {
                let (pk, sk) = Dilithium2::keypair(&mut rng).map_err(|e| {
                    EngineError::Crypto(format!("Keypair generation failed: {:?}", e))
                })?;
...
pub(crate) fn ml_dsa_sigver(group: &TestGroup, case: &TestCase) -> Result<()> {
    ...
    let pk = match DilithiumPublicKey::from_bytes(&pk_bytes) {
        ...
        Err(_) => {
            ...
            case.outputs
                .borrow_mut()
                .insert("reason".into(), "invalid public key format".into());
            return Ok(());
        }
    };
    ...
    let sig = match DilithiumSignatureData::from_bytes(&sig_bytes) {
        ...
        Err(_) => {
            ...
            case.outputs
                .borrow_mut()
                .insert("reason".into(), "invalid signature format".into());
            return Ok(());
        }
    };
