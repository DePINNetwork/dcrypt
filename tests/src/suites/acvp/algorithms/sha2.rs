...
pub(crate) fn sha2_aft(group: &TestGroup, case: &TestCase) -> Result<()> {
    ...
    let digest_hex = match algorithm.as_str() {
        ...
        _ => return Err(EngineError::InvalidData(format!("Unsupported SHA-2 variant: {}", algorithm))),
    };
...
pub(crate) fn sha2_mct(group: &TestGroup, case: &TestCase) -> Result<()> {
    ...
    let final_digest = match algorithm.as_str() {
        ...
        "SHA2-512/224" | "SHA2-512-224" | "SHA-512/224" => sha2_mct_inner::<Sha512_224>(&seed_bytes, 1000)?,
        "SHA2-512/256" | "SHA2-512-256" | "SHA-512/256" => sha2_mct_inner::<Sha512_256>(&seed_bytes, 1000)?,
        _ => return Err(EngineError::InvalidData(format!("Unsupported SHA-2 variant: {}", algorithm))),
    };
...
pub fn register(map: &mut std::collections::HashMap<DispatchKey, HandlerFn>) {
    ...
    for algo in &["SHA2-224", "SHA-224", "SHA2-256", "SHA-256", 
                  "SHA2-384", "SHA-384", "SHA2-512", "SHA-512",
                  "SHA2-512-224", "SHA-512/224", "SHA2-512/224",  // Added SHA2-512/224
                  "SHA2-512-256", "SHA-512/256", "SHA2-512/256"]  // Added SHA2-512/256
    {
