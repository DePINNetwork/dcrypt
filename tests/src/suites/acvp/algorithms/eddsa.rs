...
pub(crate) fn eddsa_keygen(group: &TestGroup, case: &TestCase) -> Result<()> {
    ...
    if !is_ed25519(&curve) {
        ...
        if is_ed448(&curve) {
            ...
            case.outputs.borrow_mut().insert("d".into(), hex::encode(&dummy_d));
            case.outputs.borrow_mut().insert("q".into(), hex::encode(&dummy_q));
            return Ok(());
        }
        return Err(EngineError::Crypto(format!("Unsupported EdDSA curve: {}", curve)));
    }
    ...
    case.outputs.borrow_mut().insert("q".into(), hex::encode(q_bytes));
...
pub(crate) fn eddsa_keyver(group: &TestGroup, case: &TestCase) -> Result<()> {
    ...
    if !is_ed25519(&curve) {
        ...
        if is_ed448(&curve) {
            ...
            case.outputs.borrow_mut().insert("reason".into(), "ED-448 not implemented".into());
            return Ok(());
        }
        return Err(EngineError::Crypto(format!("Unsupported EdDSA curve: {}", curve)));
    }
...
pub(crate) fn eddsa_siggen(group: &TestGroup, case: &TestCase) -> Result<()> {
    ...
    if !is_ed25519(&curve) {
        ...
        if is_ed448(&curve) {
            ...
            case.outputs.borrow_mut().insert("d".into(), hex::encode(&dummy_d));
            case.outputs.borrow_mut().insert("q".into(), hex::encode(&dummy_q));
            case.outputs.borrow_mut().insert("signature".into(), hex::encode(&dummy_sig));
            return Ok(());
        }
        return Err(EngineError::Crypto(format!("Unsupported EdDSA curve: {}", curve)));
    }
    ...
    if let Some(d_hex) = case.inputs.get("d").map(|v| v.as_string()) {
        ...
        if d_bytes.len() != 32 {
            return Err(EngineError::InvalidData(
                format!("Ed25519 private key must be 32 bytes, got {}", d_bytes.len())
            ));
        }
        ...
        let (public_key, secret_key) = Ed25519::keypair(&mut rng)
            .map_err(|e| EngineError::Crypto(format!("Failed to regenerate keypair from seed: {:?}", e)))?;
        ...
        case.outputs.borrow_mut().insert("q".into(), hex::encode(public_key.as_ref()));
        case.outputs.borrow_mut().insert("signature".into(), hex::encode(signature.as_ref()));
    } else {
...
pub(crate) fn eddsa_sigver(group: &TestGroup, case: &TestCase) -> Result<()> {
    ...
    if q_bytes.len() != 32 {
        case.outputs.borrow_mut().insert("testPassed".into(), "false".into());
        case.outputs.borrow_mut().insert("reason".into(), "Invalid public key size".into());
        return Ok(());
    }
    
    if sig_bytes.len() != 64 {
        case.outputs.borrow_mut().insert("testPassed".into(), "false".into());
        case.outputs.borrow_mut().insert("reason".into(), "Invalid signature size".into());
        return Ok(());
    }
