...
impl AcvpEngine for DcryptEngine {
    fn run(&self, group: &TestGroup, case: &TestCase) -> std::result::Result<(), String> {
        ...
        self.run_internal(group, case).map_err(|e| e.to_string())
    }
}
...
impl DcryptEngine {
    fn run_internal(&self, group: &TestGroup, case: &TestCase) -> Result<()> {
        ...
        let operation_key = group
            .defaults
            .get("function")
            .map(|v| v.as_string()) // Check group defaults first
            .or_else(|| {
                group
                    .params
                    .as_ref() // Then group params (as seen in ML-KEM JSON structure)
