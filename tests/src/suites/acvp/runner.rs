...
use crate::suites::acvp::model::{TestGroup, TestSuite, TestCase};
...
impl<'e, E: AcvpEngine> Runner<'e, E> {
    pub fn new(engine: &'e E) -> Self { Self { engine } }
    ...
    pub fn run_suite(&self, suite: &TestSuite) -> Result<(), String> {
        ...
        if !results.is_empty() {
            println!("Collected {} groups of results for response generation", results.len());
