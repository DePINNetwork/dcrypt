    ...
    for (i, test) in test_vectors.iter().enumerate() {
        ...
        if test.len % 8 != 0 {
            ...
            if bytes < msg.len() {
                ...
                let hash = H::digest(&truncated_msg);
...
fn test_sha3_nist_short_vectors() {
    ...
    // Path to the test vector files
    let sha3_224_path = format!("{}/sha3/SHA3_224ShortMsg.rsp", vectors_dir);
    let sha3_256_path = format!("{}/sha3/SHA3_256ShortMsg.rsp", vectors_dir);
    let sha3_384_path = format!("{}/sha3/SHA3_384ShortMsg.rsp", vectors_dir);
    let sha3_512_path = format!("{}/sha3/SHA3_512ShortMsg.rsp", vectors_dir);
    
    // Check if files exist and FAIL if they don't
    assert!(
        std::path::Path::new(&sha3_256_path).exists(),
        "Test vector file not found: {}. Please ensure the test vectors are in the correct directory.",
        sha3_256_path
    );
    
    // Run the tests
    run_sha3_tests::<Sha3_224>(&sha3_224_path, "SHA3-224");
    run_sha3_tests::<Sha3_256>(&sha3_256_path, "SHA3-256");
    run_sha3_tests::<Sha3_384>(&sha3_384_path, "SHA3-384");
    run_sha3_tests::<Sha3_512>(&sha3_512_path, "SHA3-512");
}
...
fn test_sha3_nist_long_vectors() {
    ...
    let sha3_224_path = format!("{}/sha3/SHA3_224LongMsg.rsp", vectors_dir);
    let sha3_256_path = format!("{}/sha3/SHA3_256LongMsg.rsp", vectors_dir);
    let sha3_384_path = format!("{}/sha3/SHA3_384LongMsg.rsp", vectors_dir);
    let sha3_512_path = format!("{}/sha3/SHA3_512LongMsg.rsp", vectors_dir);
...
// The actual Monte Carlo test procedure for SHA-3
...
fn test_sha3_nist_monte_vectors() {
    ...
    let sha3_224_path = format!("{}/sha3/SHA3_224Monte.rsp", vectors_dir);
    let sha3_256_path = format!("{}/sha3/SHA3_256Monte.rsp", vectors_dir);
    let sha3_384_path = format!("{}/sha3/SHA3_384Monte.rsp", vectors_dir);
    let sha3_512_path = format!("{}/sha3/SHA3_512Monte.rsp", vectors_dir);
    ...
    run_sha3_monte_tests::<Sha3_224>(&sha3_224_path, "SHA3-224");
    run_sha3_monte_tests::<Sha3_256>(&sha3_256_path, "SHA3-256");
    run_sha3_monte_tests::<Sha3_384>(&sha3_384_path, "SHA3-384");
    run_sha3_monte_tests::<Sha3_512>(&sha3_512_path, "SHA3-512");
}