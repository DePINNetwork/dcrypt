...
fn test_shake_256_empty() {
    ...
    let hash = Shake256::digest(&[]);
    assert_eq!(hex::encode(hash), expected);
}

// Modified test function that only tests FIXED output sizes
