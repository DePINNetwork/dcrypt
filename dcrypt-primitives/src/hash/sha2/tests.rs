...
fn test_sha2_nist_short_vectors() {
    ...
    let vectors_dir = format!("{}/../tests/src/vectors", base_path);
    
    // Path to the test vector files
    let sha224_path = format!("{}/sha2/SHA224ShortMsg.rsp", vectors_dir);
    let sha256_path = format!("{}/sha2/SHA256ShortMsg.rsp", vectors_dir);
    let sha384_path = format!("{}/sha2/SHA384ShortMsg.rsp", vectors_dir);
    let sha512_path = format!("{}/sha2/SHA512ShortMsg.rsp", vectors_dir);
    ...
    // Run the tests
    run_sha2_tests::<Sha224>(&sha224_path, "SHA-224");
    run_sha2_tests::<Sha256>(&sha256_path, "SHA-256");
    run_sha2_tests::<Sha384>(&sha384_path, "SHA-384");
    run_sha2_tests::<Sha512>(&sha512_path, "SHA-512");
}
...
fn test_sha2_nist_long_vectors() {
    ...
    let sha224_path = format!("{}/sha2/SHA224LongMsg.rsp", vectors_dir);
    let sha256_path = format!("{}/sha2/SHA256LongMsg.rsp", vectors_dir);
    let sha384_path = format!("{}/sha2/SHA384LongMsg.rsp", vectors_dir);
    let sha512_path = format!("{}/sha2/SHA512LongMsg.rsp", vectors_dir);
    ...
    for (i, test) in test_vectors.iter().enumerate() {
        ...
        if test.len == 0 {
            ...
            assert_eq!(hash.as_ref(), expected.as_slice(), 
                ...
                name, i, test.md, hex::encode(hash.as_ref()));
        ...
        if test.len % 8 != 0 {
            ...
            if bytes < msg.len() {
                ...
                assert_eq!(hash.as_ref(), expected.as_slice(), 
                    ...
                    name, i, test.msg, test.md, hex::encode(hash.as_ref()));
...
fn test_sha2_nist_monte_vectors() {
    ...
    let sha224_path = format!("{}/sha2/SHA224Monte.rsp", vectors_dir);
    let sha256_path = format!("{}/sha2/SHA256Monte.rsp", vectors_dir);
    let sha384_path = format!("{}/sha2/SHA384Monte.rsp", vectors_dir);
    let sha512_path = format!("{}/sha2/SHA512Monte.rsp", vectors_dir);
    ...
    run_sha2_monte_tests::<Sha224>(&sha224_path, "SHA-224");
    run_sha2_monte_tests::<Sha256>(&sha256_path, "SHA-256");
    run_sha2_monte_tests::<Sha384>(&sha384_path, "SHA-384");
    run_sha2_monte_tests::<Sha512>(&sha512_path, "SHA-512");
}
...
where H::Output: AsRef<[u8]> {
    ...
    for (i, test) in test_vectors.iter().enumerate() {
        ...
        assert_eq!(md[test.count].as_slice(), expected.as_slice(),
            "{} Monte Carlo test case {} failed.\nExpected: {}\nGot: {}", 
            name, i, test.expected, hex::encode(&md[test.count]));
    }
}