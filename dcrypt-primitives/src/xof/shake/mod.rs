//! SHAKE extendable output functions
//!
//! This module implements the SHAKE family of extendable output functions (XOFs)
//! as specified in FIPS PUB 202.
//!
//! These are distinct from the SHAKE implementations in the hash module,
//! which provide fixed-output hash function interfaces. This module provides
//! the proper XOF interface for variable-length output generation.

#[cfg(not(feature = "std"))]
use alloc::vec::Vec;
use zeroize::Zeroize;

use super::ExtendableOutputFunction;
use crate::error::{Error, Result};

// SHAKE constants
const KECCAK_ROUNDS: usize = 24;
const KECCAK_STATE_SIZE: usize = 25; // 5x5 of 64-bit words

// SHAKE rates (in bytes): r = 1600 - 2*security_level
const SHAKE128_RATE: usize = 168; // 1600 - 2*128 = 1600 - 256 = 1344 bits = 168 bytes
const SHAKE256_RATE: usize = 136; // 1600 - 2*256 = 1600 - 512 = 1088 bits = 136 bytes

// Round constants for Keccak
const RC: [u64; KECCAK_ROUNDS] = [
    0x0000000000000001, 0x0000000000008082, 0x800000000000808A, 0x8000000080008000,
    0x000000000000808B, 0x0000000080000001, 0x8000000080008081, 0x8000000000008009,
    0x000000000000008A, 0x0000000000000088, 0x0000000080008009, 0x000000008000000A,
    0x000000008000808B, 0x800000000000008B, 0x8000000000008089, 0x8000000000008003,
    0x8000000000008002, 0x8000000000000080, 0x000000000000800A, 0x800000008000000A,
    0x8000000080008081, 0x8000000000008080, 0x0000000080000001, 0x8000000080008008,
];

// Rotation offsets
const RHO: [u32; 24] = [
    1, 3, 6, 10, 15, 21, 28, 36, 45, 55, 2, 14,
    27, 41, 56, 8, 25, 43, 62, 18, 39, 61, 20, 44,
];

// Mapping from index positions to x,y coordinates in the state array
const PI: [usize; 24] = [
    10, 7, 11, 17, 18, 3, 5, 16, 8, 21, 24, 4,
    15, 23, 19, 13, 12, 2, 20, 14, 22, 9, 6, 1,
];

/// SHAKE-128 extendable output function
#[derive(Clone, Zeroize)]
pub struct ShakeXof128 {
    state: [u64; KECCAK_STATE_SIZE],
    buffer: [u8; SHAKE128_RATE],
    buffer_idx: usize,
    is_finalized: bool,
    squeezing: bool,
}

/// SHAKE-256 extendable output function
#[derive(Clone, Zeroize)]
pub struct ShakeXof256 {
    state: [u64; KECCAK_STATE_SIZE],
    buffer: [u8; SHAKE256_RATE],
    buffer_idx: usize,
    is_finalized: bool,
    squeezing: bool,
}

// Helper functions for Keccak permutation

/// Performs a full Keccak-f[1600] permutation on the state
fn keccak_f1600(state: &mut [u64; KECCAK_STATE_SIZE]) {
    // FIXED: Complete rounds implementation
    for round in 0..KECCAK_ROUNDS {
        // Theta step
        let mut c = [0u64; 5];
        for x in 0..5 {
            c[x] = state[x] ^ state[x + 5] ^ state[x + 10] ^ state[x + 15] ^ state[x + 20];
        }
        
        let mut d = [0u64; 5];
        for x in 0..5 {
            // FIXED: Correct rotation with proper indices
            d[x] = c[(x + 4) % 5] ^ c[(x + 1) % 5].rotate_left(1);
        }
        
        for y in 0..5 {
            for x in 0..5 {
                state[x + 5 * y] ^= d[x];
            }
        }
        
        // Rho and Pi steps
        let mut b = [0u64; KECCAK_STATE_SIZE];
        let mut x = 1;
        let mut y = 0;
        
        b[0] = state[0];
        
        // FIXED: Proper implementation of Rho and Pi steps
        for i in 0..24 {
            let idx = x + 5 * y;
            b[PI[i]] = state[idx].rotate_left(RHO[i]);
            
            // Update coordinates using formula from Keccak spec
            let temp = y;
            y = (2 * x + 3 * y) % 5;
            x = temp;
        }
        
        // Chi step
        for y in 0..5 {
            for x in 0..5 {
                let idx = x + 5 * y;
                state[idx] = b[idx] ^ ((!b[(x + 1) % 5 + 5 * y]) & b[(x + 2) % 5 + 5 * y]);
            }
        }
        
        // Iota step - mix in round constant
        state[0] ^= RC[round];
    }
}

/// Absorbs data into the sponge state
fn keccak_absorb(state: &mut [u64; KECCAK_STATE_SIZE], data: &[u8], rate: usize) {
    // FIXED: Correct data absorption
    for i in 0..data.len() {
        let pos = i % rate; // Position within rate-sized block
        let byte_idx = pos % 8;
        let word_idx = pos / 8;
        
        // XOR the byte into the correct position in state
        // FIXED: Proper little-endian byte positioning
        state[word_idx] ^= (data[i] as u64) << (8 * byte_idx);
    }
    
    // Apply the permutation if we've filled a complete block
    if data.len() > 0 && data.len() % rate == 0 {
        keccak_f1600(state);
    }
}

impl ShakeXof128 {
    /// Creates a new SHAKE-128 instance
    fn init() -> Self {
        ShakeXof128 {
            state: [0u64; KECCAK_STATE_SIZE],
            buffer: [0u8; SHAKE128_RATE],
            buffer_idx: 0,
            is_finalized: false,
            squeezing: false,
        }
    }
}

impl ExtendableOutputFunction for ShakeXof128 {
    fn new() -> Self {
        Self::init()
    }
    
    fn update(&mut self, data: &[u8]) -> Result<()> {
        if self.is_finalized {
            return Err(Error::InvalidParameter("Cannot update after finalization"));
        }
        
        if self.squeezing {
            return Err(Error::InvalidParameter("Cannot update after squeezing has begun"));
        }
        
        let mut idx = 0;
        
        // Fill the buffer if it has some data already
        if self.buffer_idx > 0 {
            let to_copy = (SHAKE128_RATE - self.buffer_idx).min(data.len());
            self.buffer[self.buffer_idx..self.buffer_idx + to_copy].copy_from_slice(&data[..to_copy]);
            self.buffer_idx += to_copy;
            idx = to_copy;
            
            if self.buffer_idx == SHAKE128_RATE {
                keccak_absorb(&mut self.state, &self.buffer, SHAKE128_RATE);
                self.buffer_idx = 0;
            }
        }
        
        // Process complete blocks directly from the input data
        let remaining = data.len() - idx;
        let full_blocks = remaining / SHAKE128_RATE;
        
        if full_blocks > 0 {
            // Process each full block
            for i in 0..full_blocks {
                let start = idx + i * SHAKE128_RATE;
                keccak_absorb(&mut self.state, &data[start..start + SHAKE128_RATE], SHAKE128_RATE);
            }
            idx += full_blocks * SHAKE128_RATE;
        }
        
        // Store any remaining data for next update/finalize
        if idx < data.len() {
            let remaining = data.len() - idx;
            self.buffer[self.buffer_idx..self.buffer_idx + remaining].copy_from_slice(&data[idx..]);
            self.buffer_idx += remaining;
        }
        
        Ok(())
    }
    
    fn finalize(&mut self) -> Result<()> {
        if self.is_finalized {
            return Ok(());
        }

        // Build a full-rate block, copying in any bytes we've buffered...
        let mut pad_block = [0u8; SHAKE128_RATE];
        pad_block[..self.buffer_idx]
            .copy_from_slice(&self.buffer[..self.buffer_idx]);

        // Domain separation suffix (0x1F) XORed into the very next bit‐position
        pad_block[self.buffer_idx] ^= 0x1F;

        // The final '1' bit of the multi-rate pad goes at the last bit of the block
        pad_block[SHAKE128_RATE - 1] ^= 0x80;

        // Absorb the fully-padded block (keccak_absorb will permute because len == rate)
        keccak_absorb(&mut self.state, &pad_block, SHAKE128_RATE);

        self.is_finalized = true;
        self.buffer_idx   = 0;
        Ok(())
    }
    
    fn squeeze(&mut self, output: &mut [u8]) -> Result<()> {
        if !self.is_finalized {
            self.finalize()?;
        }
        
        self.squeezing = true;
        
        let mut offset = 0;
        let rate = SHAKE128_RATE;
        
        // FIXED: Proper squeezing implementation
        while offset < output.len() {
            // If we need to generate a new block
            if self.buffer_idx >= rate {
                keccak_f1600(&mut self.state);
                self.buffer_idx = 0;
            }
            
            // If we're starting a new block, extract from state
            if self.buffer_idx == 0 {
                // Extract state to buffer - FIXED: Proper extraction
                for i in 0..(rate / 8) {
                    let lane = self.state[i];
                    for j in 0..8 {
                        if i * 8 + j < rate {
                            self.buffer[i * 8 + j] = ((lane >> (8 * j)) & 0xFF) as u8;
                        }
                    }
                }
            }
            
            // Calculate how many bytes we can copy
            let bytes_available = rate - self.buffer_idx;
            let bytes_needed = output.len() - offset;
            let bytes_to_copy = bytes_available.min(bytes_needed);
            
            // Copy bytes from buffer to output
            output[offset..offset + bytes_to_copy].copy_from_slice(
                &self.buffer[self.buffer_idx..self.buffer_idx + bytes_to_copy]
            );
            
            offset += bytes_to_copy;
            self.buffer_idx += bytes_to_copy;
        }
        
        Ok(())
    }
    
    fn squeeze_into_vec(&mut self, len: usize) -> Result<Vec<u8>> {
        let mut result = vec![0u8; len];
        self.squeeze(&mut result)?;
        Ok(result)
    }
    
    fn reset(&mut self) -> Result<()> {
        *self = Self::new();
        Ok(())
    }
    
    fn security_level() -> usize {
        128
    }
}

impl ShakeXof256 {
    /// Creates a new SHAKE-256 instance
    fn init() -> Self {
        ShakeXof256 {
            state: [0u64; KECCAK_STATE_SIZE],
            buffer: [0u8; SHAKE256_RATE],
            buffer_idx: 0,
            is_finalized: false,
            squeezing: false,
        }
    }
}

impl ExtendableOutputFunction for ShakeXof256 {
    fn new() -> Self {
        Self::init()
    }
    
    fn update(&mut self, data: &[u8]) -> Result<()> {
        if self.is_finalized {
            return Err(Error::InvalidParameter("Cannot update after finalization"));
        }
        
        if self.squeezing {
            return Err(Error::InvalidParameter("Cannot update after squeezing has begun"));
        }
        
        let mut idx = 0;
        
        // Fill the buffer if it has some data already
        if self.buffer_idx > 0 {
            let to_copy = (SHAKE256_RATE - self.buffer_idx).min(data.len());
            self.buffer[self.buffer_idx..self.buffer_idx + to_copy].copy_from_slice(&data[..to_copy]);
            self.buffer_idx += to_copy;
            idx = to_copy;
            
            if self.buffer_idx == SHAKE256_RATE {
                keccak_absorb(&mut self.state, &self.buffer, SHAKE256_RATE);
                self.buffer_idx = 0;
            }
        }
        
        // Process complete blocks directly from the input data
        let remaining = data.len() - idx;
        let full_blocks = remaining / SHAKE256_RATE;
        
        if full_blocks > 0 {
            // Process each full block
            for i in 0..full_blocks {
                let start = idx + i * SHAKE256_RATE;
                keccak_absorb(&mut self.state, &data[start..start + SHAKE256_RATE], SHAKE256_RATE);
            }
            idx += full_blocks * SHAKE256_RATE;
        }
        
        // Store any remaining data for next update/finalize
        if idx < data.len() {
            let remaining = data.len() - idx;
            self.buffer[self.buffer_idx..self.buffer_idx + remaining].copy_from_slice(&data[idx..]);
            self.buffer_idx += remaining;
        }
        
        Ok(())
    }
    
    fn finalize(&mut self) -> Result<()> {
        if self.is_finalized {
            return Ok(());
        }

        let mut pad_block = [0u8; SHAKE256_RATE];
        pad_block[..self.buffer_idx]
            .copy_from_slice(&self.buffer[..self.buffer_idx]);

        pad_block[self.buffer_idx] ^= 0x1F;
        pad_block[SHAKE256_RATE - 1] ^= 0x80;

        keccak_absorb(&mut self.state, &pad_block, SHAKE256_RATE);

        self.is_finalized = true;
        self.buffer_idx   = 0;
        Ok(())
    }
    
    fn squeeze(&mut self, output: &mut [u8]) -> Result<()> {
        if !self.is_finalized {
            self.finalize()?;
        }
        
        self.squeezing = true;
        
        let mut offset = 0;
        let rate = SHAKE256_RATE;
        
        // FIXED: Proper squeezing implementation
        while offset < output.len() {
            // If we need to generate a new block
            if self.buffer_idx >= rate {
                keccak_f1600(&mut self.state);
                self.buffer_idx = 0;
            }
            
            // If we're starting a new block, extract from state
            if self.buffer_idx == 0 {
                // Extract state to buffer - FIXED: Proper extraction
                for i in 0..(rate / 8) {
                    let lane = self.state[i];
                    for j in 0..8 {
                        if i * 8 + j < rate {
                            self.buffer[i * 8 + j] = ((lane >> (8 * j)) & 0xFF) as u8;
                        }
                    }
                }
            }
            
            // Calculate how many bytes we can copy
            let bytes_available = rate - self.buffer_idx;
            let bytes_needed = output.len() - offset;
            let bytes_to_copy = bytes_available.min(bytes_needed);
            
            // Copy bytes from buffer to output
            output[offset..offset + bytes_to_copy].copy_from_slice(
                &self.buffer[self.buffer_idx..self.buffer_idx + bytes_to_copy]
            );
            
            offset += bytes_to_copy;
            self.buffer_idx += bytes_to_copy;
        }
        
        Ok(())
    }
    
    fn squeeze_into_vec(&mut self, len: usize) -> Result<Vec<u8>> {
        let mut result = vec![0u8; len];
        self.squeeze(&mut result)?;
        Ok(result)
    }
    
    fn reset(&mut self) -> Result<()> {
        *self = Self::new();
        Ok(())
    }
    
    fn security_level() -> usize {
        256
    }
}

#[cfg(test)]
mod tests;