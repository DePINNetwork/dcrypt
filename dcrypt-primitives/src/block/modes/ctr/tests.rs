...
fn test_aes_ctr_nist_vectors() {
    ...
    let vectors_dir = format!("{}/../dcrypt-test/src/vectors/ctr", base_path);
...
fn test_aes_ctr() {
    ...
    let decrypted = ctr.decrypt(&ciphertext);
...
fn test_ctr_with_custom_params() {
    ...
    let cipher = Aes128::new(&key);
    let mut ctr = Ctr::with_counter_params(cipher, &nonce, CounterPosition::Prefix, 8);
    ...
    let cipher = Aes128::new(&key);
    let mut ctr = Ctr::with_counter_params(cipher, &nonce, CounterPosition::Custom(4), 4);
...
fn test_ctr_counter_overflow() {
    ...
    let cipher = Aes128::new(&key);
    let mut ctr = Ctr::with_counter_params(cipher, &nonce, CounterPosition::Postfix, 1);
    ...
    let ciphertext = ctr.encrypt(&large_plaintext);
...
fn test_ctr_reset() {
    ...
    // Encrypt some data
    let plaintext1 = vec![0xAA; 16];
    let _ciphertext1 = ctr.encrypt(&plaintext1);
    
    // Reset with same nonce but different offset
    ctr.reset(&nonce, 10);
    
    // Encrypt more data
    let plaintext2 = vec![0xBB; 16];
    let ciphertext2 = ctr.encrypt(&plaintext2);
    
    // Re-create with same parameters for verification
    let cipher = Aes128::new(&key);
    let mut ctr_verify = Ctr::new(cipher, &nonce);
    ctr_verify.set_counter(10);
    
    let ciphertext_verify = ctr_verify.encrypt(&plaintext2);
    assert_eq!(ciphertext2, ciphertext_verify);
}
...
fn test_ctr_long_message() {
    ...
    // Decrypt and verify
    let mut ctr = Ctr::new(Aes128::new(&key), &nonce);
    let decrypted = ctr.decrypt(&ciphertext);
...
fn test_different_counter_sizes() {
    ...
    let mut ctr = Ctr::with_counter_params(cipher, &nonce[..11], CounterPosition::Postfix, 1);
    let ciphertext1 = ctr.encrypt(&plaintext);
    
    // 2-byte counter
    let cipher = Aes128::new(&key);
    let mut ctr = Ctr::with_counter_params(cipher, &nonce[..10], CounterPosition::Postfix, 2);
    let ciphertext2 = ctr.encrypt(&plaintext);
    
    // 8-byte counter
    let cipher = Aes128::new(&key);
    let mut ctr = Ctr::with_counter_params(cipher, &nonce[..8], CounterPosition::Postfix, 8);
    let ciphertext3 = ctr.encrypt(&plaintext);
    ...
    let cipher = Aes128::new(&key);
    let mut ctr = Ctr::with_counter_params(cipher, &nonce[..11], CounterPosition::Postfix, 1);
    let decrypted = ctr.decrypt(&ciphertext1);
    assert_eq!(decrypted, plaintext);
    
    let cipher = Aes128::new(&key);
    let mut ctr = Ctr::with_counter_params(cipher, &nonce[..10], CounterPosition::Postfix, 2);
    let decrypted = ctr.decrypt(&ciphertext2);
    assert_eq!(decrypted, plaintext);
    
    let cipher = Aes128::new(&key);
    let mut ctr = Ctr::with_counter_params(cipher, &nonce[..8], CounterPosition::Postfix, 8);
    let decrypted = ctr.decrypt(&ciphertext3);
    assert_eq!(decrypted, plaintext);
}