...
use super::field::{sqrt, FieldElement};
...
mod tests {
    ...
    fn test_basic_scalar_mult_stays_on_curve() {
        ...
        assert!(
            p1.is_on_curve(),
            "Point after scalar mult by 1 not on curve!"
        );
        ...
        assert!(
            p2.is_on_curve(),
            "Point after scalar mult by 2 not on curve!"
        );

        // Test that they can be compressed and decompressed
        let c1 = p1.compress();
        assert!(
            c1.decompress().is_some(),
            "Failed to decompress point after scalar mult by 1"
        );

        let c2 = p2.compress();
        assert!(
            c2.decompress().is_some(),
            "Failed to decompress point after scalar mult by 2"
        );
    }
}
