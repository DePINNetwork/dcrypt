...
impl AsRef<[u8]> for EcdsaP192PublicKey { fn as_ref(&self) -> &[u8] { &self.0 } }
impl AsMut<[u8]> for EcdsaP192PublicKey { fn as_mut(&mut self) -> &mut [u8] { &mut self.0 } }
impl AsRef<[u8]> for EcdsaP192SecretKey { fn as_ref(&self) -> &[u8] { &self.bytes } }
impl AsMut<[u8]> for EcdsaP192SecretKey { fn as_mut(&mut self) -> &mut [u8] { &mut self.bytes } }
impl AsRef<[u8]> for EcdsaP192Signature { fn as_ref(&self) -> &[u8] { &self.0 } }
impl AsMut<[u8]> for EcdsaP192Signature { fn as_mut(&mut self) -> &mut [u8] { &mut self.0 } }
...
impl SignatureTrait for EcdsaP192 {
    ...
    fn verify(message: &[u8], signature: &Self::SignatureData, public_key: &Self::PublicKey) -> ApiResult<()> {
        ...
        let r = ec::Scalar::new(r_bytes).map_err(|_| ApiError::InvalidSignature {
            context: "ECDSA-P192 verify", #[cfg(feature = "std")] message: "Invalid r component".to_string(),
        })?;
        let s = ec::Scalar::new(s_bytes).map_err(|_| ApiError::InvalidSignature {
            context: "ECDSA-P192 verify", #[cfg(feature = "std")] message: "Invalid s component".to_string(),
        })?;
