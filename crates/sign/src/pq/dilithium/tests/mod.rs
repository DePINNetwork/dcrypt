...
fn test_dilithium2_sign_verify() {
    ...
    let c = sample_challenge_c::<Dilithium2Params>(
        &c_tilde_sig,
        Dilithium2Params::TAU_PARAM as u32,
    ).unwrap();
    ...
    assert_eq!(c_tilde_sig, c_tilde_recomputed, "Challenge seeds don't match");
    ...
    assert!(identity_holds, "Algebraic identity w' = w + ct0 - cs2 doesn't hold");
}
...
fn test_decompose() {
    ...
    assert_eq!((a1 as i32 * alpha as i32 + a0) % DILITHIUM_Q as i32, (a % DILITHIUM_Q) as i32);
...
fn test_sample_challenge() {
    ...
    for (tau, seed_size) in test_cases {
        ...
        let non_zero_count = c_poly.coeffs.iter()
            .filter(|&&c| c != 0)
            .count();
...
fn test_sample_uniform_gamma1() {
    ...
    for poly in &pv.polys {
        for &coeff in &poly.coeffs {
            ...
            let bound = gamma1 as i32 - Dilithium2Params::BETA_PARAM as i32 - Dilithium2Params::ETA_S1S2 as i32;
...
fn test_decompose_properties() {
    ...
    for _ in 0..1000 {
        ...
        let reconstructed = (((a1 as i64) * (alpha as i64) + (a0 as i64))
            .rem_euclid(DILITHIUM_Q as i64)) as u32;
...
fn test_hint_system_complete() {
    ...
    pv_z.polys[0].coeffs[0] = create_boundary_crossing_z(
        pv_r.polys[0].coeffs[0], ALPHA, GAMMA2, true
    );
    
    // Test case 2: Force crossing -γ₂ boundary  
    pv_r.polys[0].coeffs[1] = 7654321 % DILITHIUM_Q;
    pv_z.polys[0].coeffs[1] = create_boundary_crossing_z(
        pv_r.polys[0].coeffs[1], ALPHA, GAMMA2, false
    );
    ...
    // Generate hints
    let (hint_vec, hint_count) = super::arithmetic::make_hint_polyveck::<Dilithium2Params>(&pv_r, &pv_z)
    ...
    assert!(hint_count <= Dilithium2Params::OMEGA_PARAM as usize, "Hint count exceeds ω");
    
    // Verify hints at expected positions
    assert_eq!(hint_vec.polys[0].coeffs[0], 1, "Position 0 should have hint (crossed +γ₂)");
    assert_eq!(hint_vec.polys[0].coeffs[1], 1, "Position 1 should have hint (crossed -γ₂)");
    assert_eq!(hint_vec.polys[0].coeffs[2], 0, "Position 2 should not have hint (no crossing)");
    ...
    assert_eq!(mismatches, 0, "Found {} mismatches in hint recovery", mismatches);
}
...
fn test_hint_system_rejects_large_gaps() {
    ...
    assert!(result.is_err(), "make_hint_polyveck should reject large bucket gaps");
    
    if let Err(e) = result {
        // Just check that it's an error - the exact error type may vary
        // The important thing is that it rejects the large gap
        let error_string = format!("{:?}", e);
        assert!(error_string.contains("Highbit change exceeds ±1 bucket") || 
                error_string.contains("SignatureGeneration"),
                "Expected error about highbit bucket constraint, got: {}", error_string);
    }
}
...
fn test_hint_identity_comprehensive() {
    ...
    for (r, z, label) in cases {
        ...
        if z.abs() >= 100 { continue; } // Much smaller constraint than γ₂
        ...
        for &z in z_values.iter() {
            if z.abs() < gamma2 as i32 {
                ...
                assert_eq!(recovered, expected,
                    "UseHint property failed for r={} ({}*alpha), z={}", r, k, z);
            }
        }
    }
}
