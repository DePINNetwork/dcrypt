...
fn decompress_poly(data: &[u8], d: usize) -> AlgoResult<Polynomial<KyberPolyModParams>> {
    ...
    if d == 4 {
        ...
        while coeff_idx < KyberPolyModParams::N && byte_idx + KYBER_D5_BYTES_PER_CHUNK <= data.len() {
            ...
            poly.coeffs[coeff_idx + 3] = decompress_coeff(((b[1] >> 7) | ((b[2] & 0x0F) << 1)) as u32, d);
            poly.coeffs[coeff_idx + 4] = decompress_coeff(((b[2] >> 4) | ((b[3] & 0x01) << 4)) as u32, d);
        ...
        while coeff_idx < KyberPolyModParams::N && byte_idx + KYBER_D11_BYTES_PER_CHUNK <= data.len() {
            ...
            poly.coeffs[coeff_idx + 3] = decompress_coeff(((b[4] as u32) >> 1) | ((b[5] as u32 & 0x0F) << 7), d);
            poly.coeffs[coeff_idx + 4] = decompress_coeff(((b[5] as u32) >> 4) | ((b[6] as u32 & 0x7F) << 4), d);
...
fn compress_coeff(coeff: u32, d: usize) -> u32 {
    ((((coeff as u64) << d) + (KyberPolyModParams::Q as u64 / 2)) / (KyberPolyModParams::Q as u64)) as u32 & ((1 << d) - 1)
}
...
pub fn unpack_ciphertext<P: KyberParams>(bytes: &[u8]) -> AlgoResult<CpaCiphertextInner<P>> {
    ...
    // Decompress v
    let v = decompress_poly(&bytes[u_compressed_bytes..u_compressed_bytes + v_compressed_bytes], P::DV)?;
