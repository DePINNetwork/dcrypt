...
fn test_b283k_kem_basic_flow() {
    ...
    // Encapsulate
    let (ciphertext, shared_secret_sender) = EcdhB283k::encapsulate(&mut rng, &recipient_pk).unwrap();
    ...
    assert_eq!(recipient_pk.as_ref().len(), ec_b283k::B283K_POINT_COMPRESSED_SIZE);
    assert_eq!(recipient_sk.as_ref().len(), ec_b283k::B283K_SCALAR_SIZE);
    assert_eq!(ciphertext.as_ref().len(), ec_b283k::B283K_POINT_COMPRESSED_SIZE);
    assert_eq!(shared_secret_sender.as_ref().len(), ec_b283k::B283K_KEM_SHARED_SECRET_KDF_OUTPUT_SIZE);
}
...
fn test_b283k_kem_tampered_ciphertext() {
    ...
    let (mut ciphertext, shared_secret_sender) = EcdhB283k::encapsulate(&mut rng, &public_key).expect("Encapsulation failed");
