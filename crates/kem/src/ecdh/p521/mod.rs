// File: crates/kem/src/ecdh/p521.rs
...
use algorithms::ec::p521 as ec_p521;
...
impl AsRef<[u8]> for EcdhP521PublicKey { fn as_ref(&self) -> &[u8] { &self.0 } }
impl AsMut<[u8]> for EcdhP521PublicKey { fn as_mut(&mut self) -> &mut [u8] { &mut self.0 } }
impl AsRef<[u8]> for EcdhP521SecretKey { fn as_ref(&self) -> &[u8] { self.0.as_ref() } }
impl AsMut<[u8]> for EcdhP521SecretKey { fn as_mut(&mut self) -> &mut [u8] { self.0.as_mut() } }
impl AsRef<[u8]> for EcdhP521SharedSecret { fn as_ref(&self) -> &[u8] { self.0.as_ref() } }
impl AsMut<[u8]> for EcdhP521SharedSecret { fn as_mut(&mut self) -> &mut [u8] { self.0.as_mut() } }
impl AsRef<[u8]> for EcdhP521Ciphertext { fn as_ref(&self) -> &[u8] { &self.0 } }
impl AsMut<[u8]> for EcdhP521Ciphertext { fn as_mut(&mut self) -> &mut [u8] { &mut self.0 } }
...
impl Kem for EcdhP521 {
    ...
    fn keypair<R: CryptoRng + RngCore>(rng: &mut R) -> ApiResult<Self::KeyPair> {
        ...
        let (sk_scalar, pk_point) = ec_p521::generate_keypair(rng)
    ...
    fn encapsulate<R: CryptoRng + RngCore>(
        ...
        let (ephemeral_scalar, ephemeral_point) = ec_p521::generate_keypair(rng)
            .map_err(|e| ApiError::from(KemError::from(e)))?;
        ...
        let mut kdf_ikm = Vec::with_capacity(
            ec_p521::P521_FIELD_ELEMENT_SIZE + 
            2 * ec_p521::P521_POINT_COMPRESSED_SIZE  // Using compressed size
        );
        ...
        let ss_bytes = ec_p521::kdf_hkdf_sha512_for_ecdh_kem(&kdf_ikm, info.as_deref())
            .map_err(|e| ApiError::from(KemError::from(e)))?;
