...
impl Drop for Blake2b { fn drop(&mut self) { self.zeroize(); } }
...
impl Blake2b {
    ...
    pub fn with_output_size(out_len: usize) -> Self {
        ...
        Self { h, t:[0;2], f:[0;2], buf:[0;BLAKE2B_BLOCK_SIZE], buf_len:0,
               out_len, key:None, is_keyed:false }
    }
    ...
    pub fn with_parameter_block(param: [u8; 64], out_len: usize) -> Self {
        ...
        Self {
            h, t:[0;2], f:[0;2], buf:[0;BLAKE2B_BLOCK_SIZE], buf_len:0,
            out_len, key:None, is_keyed:false
        }
    }
    ...
    fn finalize_internal(&mut self) -> Result<Vec<u8>> {
        ...
        for &w in &self.h { out.extend_from_slice(&w.to_le_bytes()); }
...
impl HashFunction for Blake2b {
    ...
    fn new() -> Self { Blake2b::with_output_size(BLAKE2B_MAX_OUTPUT_SIZE) }

    fn update(&mut self, input: &[u8]) -> Result<&mut Self> {
        self.update_internal(input)?; Ok(self)
    }
    ...
    fn output_size() -> usize { Self::Algorithm::OUTPUT_SIZE }
    fn block_size() -> usize { Self::Algorithm::BLOCK_SIZE }
    fn name() -> String { Self::Algorithm::ALGORITHM_ID.to_string() }
}
