...
use crate::ec::p521::{
    constants::{P521_FIELD_ELEMENT_SIZE, P521_POINT_UNCOMPRESSED_SIZE, P521_POINT_COMPRESSED_SIZE}, 
    field::FieldElement,
    scalar::Scalar,
};
...
impl Point {
    ...
    pub fn new_uncompressed(x: &[u8; P521_FIELD_ELEMENT_SIZE], y: &[u8; P521_FIELD_ELEMENT_SIZE]) -> Result<Self> {
        ...
        if !Self::is_on_curve(&x_fe, &y_fe) {
            return Err(Error::param("P-521 Point", "Point coordinates do not satisfy curve equation"));
        }
        ...
        match (bytes[0], bytes.len()) {
            (0x00, P521_POINT_UNCOMPRESSED_SIZE) => {
                ...
                if bytes.iter().all(|&b| b == 0) {
                    ...
                    Err(Error::param("P-521 Point", "Invalid identity point encoding"))
                }
            },
            ...
            _ => Err(Error::param("P-521 Point", "Unknown or malformed point format")),
        }
    }
    ...
    pub fn deserialize_uncompressed(bytes: &[u8]) -> Result<Self> {
        ...
        if bytes[0] != 0x04 {
            return Err(Error::param("P-521 Point", "Invalid uncompressed point format (expected 0x04 prefix)"));
        }
    ...
    pub fn deserialize_compressed(bytes: &[u8]) -> Result<Self> {
        validate::length("P-521 Compressed Point", bytes.len(), P521_POINT_COMPRESSED_SIZE)?;
        ...
        if tag != 0x02 && tag != 0x03 {
            return Err(Error::param("P-521 Point", "Invalid compressed point prefix (expected 0x02 or 0x03)"));
        }
