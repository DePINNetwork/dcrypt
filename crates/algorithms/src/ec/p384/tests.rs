...
fn test_b_parameter_validation() {
    ...
    assert_eq!(b_fe.to_bytes(), b_be, "NIST_P384.b should round-trip through FieldElement");
}
...
mod edge_case_vectors {
    ...
    fn test_scalar_edge_cases() -> Result<()> {
        // Test scalar = 1
        let one_bytes = [0u8; 47].iter().chain([1u8].iter()).cloned().collect::<Vec<_>>();
        ...
        // Test scalar = 2
        let two_bytes = [0u8; 47].iter().chain([2u8].iter()).cloned().collect::<Vec<_>>();
