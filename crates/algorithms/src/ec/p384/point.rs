...
use crate::ec::p384::{
    constants::{P384_FIELD_ELEMENT_SIZE, P384_POINT_UNCOMPRESSED_SIZE, P384_POINT_COMPRESSED_SIZE}, 
    field::FieldElement,
    scalar::Scalar,
};
...
impl Point {
        ...
        match (bytes[0], bytes.len()) {
            (0x00, P384_POINT_UNCOMPRESSED_SIZE) => {
                ...
                if bytes.iter().all(|&b| b == 0) {
                    ...
                    Err(Error::param("P-384 Point", "Invalid identity point encoding"))
                }
            },
            ...
            _ => Err(Error::param("P-384 Point", "Unknown or malformed point format")),
        }
    }
    ...
    pub fn deserialize_uncompressed(bytes: &[u8]) -> Result<Self> {
        ...
        if bytes[0] != 0x04 {
            return Err(Error::param("P-384 Point", "Invalid uncompressed point format (expected 0x04 prefix)"));
        }
    ...
    /// 
    /// The identity point is encoded as 49 zero bytes for consistency
    ...
    pub fn deserialize_compressed(bytes: &[u8]) -> Result<Self> {
        validate::length("P-384 Compressed Point", bytes.len(), P384_POINT_COMPRESSED_SIZE)?;
        ...
        if tag != 0x02 && tag != 0x03 {
            return Err(Error::param("P-384 Point", "Invalid compressed point prefix (expected 0x02 or 0x03)"));
        }
        ...
        // Forward any parser failure (e.g. x ≥ p) as the generic
        // "quadratic non-residue" error so consumers can only observe a
        // single class of failure for invalid compressed points.  This is
        // also what the test-suite checks for via the substring
        // "non-residue".
        ...
        let rhs = {
            ...
            let b = FieldElement::from_bytes(&NIST_P384.b).unwrap();
