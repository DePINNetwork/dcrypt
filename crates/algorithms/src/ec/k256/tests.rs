    ...
    for _ in 0..50 {
        ...
        if let Ok(x) = FieldElement::from_bytes(&bytes) {
            ...
            if let Some(sqrt_result) = x_squared.sqrt() {
                ...
                assert_eq!(sqrt_result.square(), x_squared, "sqrt consistency check failed");
            }
        }
    }
}
...
fn test_field_arithmetic_properties() {
    ...
    for _ in 0..20 {
        ...
        if let (Ok(a), Ok(b), Ok(c)) = (
            ...
            assert_eq!(a.add(&b).add(&c), a.add(&b.add(&c)), "Addition not associative");
            
            // Commutativity: a * b = b * a
            assert_eq!(a.mul(&b), b.mul(&a), "Multiplication not commutative");
            
            // Associativity: (a * b) * c = a * (b * c)
            assert_eq!(a.mul(&b).mul(&c), a.mul(&b.mul(&c)), "Multiplication not associative");
            
            // Distributivity: a * (b + c) = a * b + a * c
            assert_eq!(a.mul(&b.add(&c)), a.mul(&b).add(&a.mul(&c)), "Multiplication not distributive");
            
            // Identity: a + 0 = a
            assert_eq!(a.add(&FieldElement::zero()), a, "Zero not additive identity");
            
            // Identity: a * 1 = a
            assert_eq!(a.mul(&FieldElement::one()), a, "One not multiplicative identity");
...
fn test_edge_cases() {
    ...
    assert_eq!(compressed, [0u8; 33], "Identity should compress to all zeros");
    
    let decompressed = Point::deserialize_compressed(&compressed).unwrap();
    assert!(decompressed.is_identity(), "Decompressed identity should be identity");
