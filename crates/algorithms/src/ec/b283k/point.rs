...
use crate::ec::b283k::{
    constants::{B283K_FIELD_ELEMENT_SIZE, B283K_POINT_UNCOMPRESSED_SIZE, B283K_POINT_COMPRESSED_SIZE}, 
    field::FieldElement,
    scalar::Scalar,
};
...
impl Point {
    ...
    pub fn new_uncompressed(x: &[u8; B283K_FIELD_ELEMENT_SIZE], y: &[u8; B283K_FIELD_ELEMENT_SIZE]) -> Result<Self> {
        let x_fe = FieldElement::from_bytes(x)?;
        let y_fe = FieldElement::from_bytes(y)?;
        if !Self::is_on_curve(&x_fe, &y_fe) {
            return Err(Error::param("B283k Point", "Point coordinates do not satisfy curve equation"));
        }
    ...
    pub fn deserialize_compressed(bytes: &[u8]) -> Result<Self> {
        validate::length("B283k Compressed Point", bytes.len(), B283K_POINT_COMPRESSED_SIZE)?;
        if bytes.iter().all(|&b| b == 0) { return Ok(Self::identity()); }
        let tag = bytes[0];
        if tag != 0x02 && tag != 0x03 {
            return Err(Error::param("B283k Point", "Invalid compressed point prefix"));
        }
        let mut x_bytes = [0u8; B283K_FIELD_ELEMENT_SIZE];
        x_bytes.copy_from_slice(&bytes[1..]);
        let x = FieldElement::from_bytes(&x_bytes)?;
        if x.is_zero() {
            return Ok(Point { is_identity: Choice::from(0), x, y: FieldElement::one().sqrt() });
        }
        ...
        let y = x.mul(&z);
        Ok(Point { is_identity: Choice::from(0), x, y })
    }
    ...
    pub fn double(&self) -> Self {
        if self.is_identity() || self.x.is_zero() { return Self::identity(); }
        ...
        Point { is_identity: Choice::from(0), x: x2, y: y2 }
    }
